...
import me.jfenn.wakeMeUp.job.DemoJobCreator;
import me.jfenn.wakeMeUp.services.TimerService;
import me.jfenn.wakeMeUp.utils.CrashUtils;
...
public class Alarmio extends Application implements Player.EventListener {
   public static final int THEME_DAY_NIGHT = 0;
    ...
    private static SunriseSunsetCalculator sunsetCalculator;

    private Ringtone currentRingtone;

    private List<AlarmData> alarms;
    private List<AlarmData> unsortedAlarms;
    ...
    public void onCreate() {
        ...
        Log.d(MainActivity.TAG,"alarmio timerLength "+timerLength);
        for (int id = 0; id < timerLength; id++) {
            TimerData timer = new TimerData(id, this);
            if (timer.isSet()) timers.add(timer);
        }

        Log.d(MainActivity.TAG,"alarmio timerLength2 "+timers.size());
    ...
    public void setAlarms() {
        ...
        unsortedAlarms=new ArrayList<>();
        unsortedAlarms.addAll(alarms);
        Collections.sort(alarms, new Comparator<AlarmData>() {
            @Override
    ...
    public void removeAlarm(AlarmData alarm) {
        ...
        refreshAlarms(alarm,true);

    }

    public void refreshAlarms(AlarmData alarm,boolean remove) {
        int index2=unsortedAlarms.indexOf(alarm);
         if(remove)unsortedAlarms.remove(index2);
         for (int i = index2; i < unsortedAlarms.size(); i++) {
            unsortedAlarms.get(i).onIdChanged(i, this,false);
        }
        alarms.clear();
        alarms.addAll(unsortedAlarms);
        Collections.sort(alarms, new Comparator<AlarmData>() {
            @Override
            public int compare(AlarmData lhs, AlarmData rhs) {
                int r=(rhs.time.get(Calendar.HOUR_OF_DAY)*60+rhs.time.get(Calendar.MINUTE));
    ...
    public void onTimerCountChanged() {
        ...
        int timerLength = PreferenceData.TIMER_LENGTH.getValue(this);
        Log.d(MainActivity.TAG,"alarmio onTimerCountChanged "+timerLength+" timerssize "+timers.size());
    ...
    public void onIdChanged(int from, int to) {
        AlarmData f=alarms.get(from);
        AlarmData t=alarms.get(to);
    ...
    public List<AlarmData> getUnsortedAlarms() {
        return unsortedAlarms;
    }

    public void addUnsortedAlarm(AlarmData alarm) {
        unsortedAlarms.add(alarm);
    }
