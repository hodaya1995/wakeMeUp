...
import me.jfenn.wakeMeUp.interfaces.Subscribblable;

public class ProgressTextViewSmall extends View implements Subscribblable {
    ...
    public ProgressTextViewSmall(Context context) {
        this(context, null);
    }

    public ProgressTextViewSmall(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ProgressTextViewSmall(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        padding = DimenUtils.dpToPx(1);
        ...
        textPaint.setTextSize(DimenUtils.spToPx(0));
        ...
        if (animate) {
            ...
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator valueAnimator) {
                    ProgressTextViewSmall.this.progress = (long) (float) valueAnimator.getAnimatedValue();
                    invalidate();
                }
            });
            animator.start();
    ...
    public void setMaxProgress(long maxProgress, boolean animate) {
        if (animate) {
            ...
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                @Override
                public void onAnimationUpdate(ValueAnimator valueAnimator) {
                    ProgressTextViewSmall.this.maxProgress = (long) (float) valueAnimator.getAnimatedValue();
                    invalidate();
                }
            });
            animator.start();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int size = DimenUtils.dpToPx(28);//getMeasuredWidth();
        setMeasuredDimension(size, size);
    }

    @Override
    protected void onDraw(Canvas canvas) {
        int size = DimenUtils.dpToPx(28);//Math.min(canvas.getWidth(), canvas.getHeight());
        int sidePadding = 0;//padding * 3;
        float radius=(size / 2) - sidePadding;
        float x=size / 2;
        float y=size / 2;
        Paint paint=progress > maxProgress && maxProgress > 0 ? linePaint : backgroundPaint;
        canvas.drawLine(x-radius,y-0.75f*radius,x-0.5f*radius,y-1.25f*radius,paint);
        canvas.drawLine(x+0.5f*radius,y-1.25f*radius,x+radius,y-0.75f*radius,paint);
