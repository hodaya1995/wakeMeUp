...
public enum PreferenceData {
    ...
    SLOW_WAKE_UP(false),
    SNOOZE_NOTIFICATION(true),
    ALARM_NOTIFICATION(false),
    ALLOW_SHUT_UP_WITH_BUTTONS(true),
    ...
    ALARM_WAS_ENABLED("%d/ALARM_WAS_ENABLED", true),
    ...
    TIMER_END_TIME("%d/TIMER_END_TIME", (long)0),
    TIMER_VIBRATE("%d/TIMER_VIBRATE", true),
    TIMER_SOUND("%d/TIMER_SOUND", ""),
    TIME_ZONE_ENABLED("%s/TIME_ZONE_ENABLED", false),
    ID("%d/ID",0),
    TIME_TO_WAKE("%d/TIME_TO_WAKE",10),
    TIME_TO_WAKE_P("%d/TIME_TO_WAKE_P",10),
    MONEY("%s/MONEY","7.2"),
    NEED_VISIBILITY("%d/NEED_VISIBILITY",false),
    POSITION("%d/POSITION",0)
    , CONSTANT_TIME("%d/CONSTANT_TIME",(long) 0),

    SNOOZE_DURATION(300000L),
    RING_DURATION(300000L),
    BUILT_IN_SPEAKER("%s/BUILT_IN_SPEAKER", true),
    SYSYTEM_VOLUME("%s/SYSYTEM_VOLUME", true),
    LANGUAGE("%d/LANGUAGE", null),
    VOLUME("%d/VOLUME",100),
    STEP("%d/STEP",2),

    CALANDAR("%d/CALANDAR",false),
    GMAIL("%d/GMAIL",false),
    WIFI(false),
    IS_PUNISHED("%d/IS_PUNISHED", true);
        ...
        if (type instanceof Object[] && prefs.contains(name + "-length")) {
            try {
                ...
                for (int i = 0; i < array.length; i++) {
                    if (array instanceof Boolean[])
                        ...
                        array[i] = prefs.contains(name + "-" + i) ? prefs.getFloat(name + "-" + i, 7.2f) : null;
            ...
            try {
                if (type instanceof Boolean)
                    ...
                    return (T) new Float(prefs.getFloat(name, (Float) defaultValue));
                ...
                if (type instanceof Long) {
                    ...
                    try {
                        return (T) new Float((int) prefs.getFloat(name, (Float) defaultValue));
                    } catch (ClassCastException ignored) {
                    }
                }
        ...
        if (value == null)
            ...
            for (int i = 0; i < array.length; i++) {
                ...
                if (item instanceof Boolean)
                    ...
                    editor.putFloat(name + "-" + i, (Float) item);
